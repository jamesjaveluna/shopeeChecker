...
function addScore($category, $name, $desc, $result, $type, $raw, &$json) {
    ...
    $json['result'][] = $scoreArray;
}

function create_result_json($status, $result_data){
    global $stats, $catStat;
    $json = array(
        'status' => $status,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($resultID != 'Invalid' && $shopeeResultData != false){
        ...
        $json = create_result_json('processing', $scoreArray);
        ...
        for ($retry_count = 0; $retry_count < $max_retry_count; $retry_count++) {
            ...
            addScore('summary', 'Lazada Score', 'Rates from Lazada Users.', 'Good', 3, '4/5', $json);
            addScore('summary', 'Zalora Score', 'Rates from Zalora Users.', 'Excellent', 4, '4.5/5', $json);
            ...
            $json = create_result_json('done', $scoreArray);
